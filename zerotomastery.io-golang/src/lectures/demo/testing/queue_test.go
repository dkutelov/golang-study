...
func TestNext(t *testing.T) {
	...
	for i := 0; i < 3; i++ {
		...
		if !ok {
			t.Errorf("should be able to get the item from the queue")
		}

		if item != i {
			t.Errorf("got the item in wrong order: %v, want: %v", item, i)
		}
	}
	...
	if ok {
		t.Errorf("Should be not more items in the queue")
	}
}