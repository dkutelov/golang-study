...
func automate(item WarehouseAutomator) {
	...
	fmt.Printf("Ship %v via %v conveyor\n", item, item.Ship())
}
...
func main() {
	// mgrAcct := ManagerAccount{Account{2, 20, "Cassio"}}

	// fmt.Println(mgrAcct)
	// fmt.Println(mgrAcct.GetBalance())
	// fmt.Println(mgrAcct.accountId)
	// fmt.Println(mgrAcct.Account.String())
